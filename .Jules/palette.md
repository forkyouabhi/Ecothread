...
## 2024-05-18 - Immediate Visual Feedback for Async Actions
**Learning:** The 'Add to Cart' functionality had a noticeable delay due to network requests, but the button only updated text without indicating an active process, leading to potential duplicate clicks or perceived unresponsiveness. Also, `button.innerText` overwrites internal HTML like icons/spinners, whereas `button.innerHTML` is required to insert visual components like Bootstrap spinners dynamically.
**Action:** Use `button.innerHTML` when modifying button states to include spinners (like Bootstrap's `.spinner-border`) alongside text to provide immediate visual feedback for async actions.
