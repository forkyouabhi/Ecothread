...
## 2026-05-28 - Bootstrap Form Validation Needs Custom JS
**Learning:** Found that Bootstrap forms utilizing the `needs-validation` class require a custom client-side JavaScript snippet to intercept form submissions and apply the `was-validated` class; without this, inline `.invalid-feedback` UI messages will not be triggered when users submit invalid forms.
**Action:** When using Bootstrap's `needs-validation` class on a form to show custom validation feedback, always include the requisite client-side JavaScript to intercept the submission and append the `was-validated` class.
