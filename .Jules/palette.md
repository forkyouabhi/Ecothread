...
## 2025-02-28 - Form Validation Feedback Requires JS Snippet
**Learning:** Bootstrap forms using the `needs-validation` class in this project require a custom client-side JavaScript snippet to intercept submissions and apply the `was-validated` class. Without this script, inline `.invalid-feedback` messages are not displayed to users, leading to confusing silent failures on form submission.
**Action:** Always verify that the standard Bootstrap 5 validation JS snippet is present on pages containing forms with the `needs-validation` class to ensure actionable error feedback is visible to users.
