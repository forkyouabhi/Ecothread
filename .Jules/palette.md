...
## 2024-05-23 - Bootstrap Form Validation Requires Client-Side JS
**Learning:** Bootstrap `.needs-validation` forms containing `.invalid-feedback` elements require a custom client-side JavaScript snippet to intercept form submissions and apply the `was-validated` class. Without this script, the inline error messages will never display, even if required fields are missing.
**Action:** When adding or verifying form validation in Bootstrap-based views (like `.hbs` files), ensure the `was-validated` script is included on the page or globally.
