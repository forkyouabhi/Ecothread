...
## 2024-07-26 - Client-Side Validation Requires JS with `novalidate`
**Learning:** Found that a form (`views/addproduct.hbs`) used the `needs-validation` class and `novalidate` attribute (which disables standard browser tooltips) to rely on Bootstrap's inline `.invalid-feedback` classes. However, without the corresponding JavaScript to intercept the submit event and add the `.was-validated` class to the form, the form submission proceeds directly without showing any validation feedback to the user, creating a confusing experience where required fields fail silently.
**Action:** When using Bootstrap's `needs-validation` with `novalidate`, always ensure the standard Bootstrap JavaScript validation snippet is included on the page to properly intercept form submissions, apply the `.was-validated` class, and display the inline `.invalid-feedback` messages.
