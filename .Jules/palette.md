...
## 2024-07-28 - Bootstrap Inline Validation Feedback
**Learning:** Bootstrap forms utilizing the `needs-validation` class require a custom client-side JavaScript snippet to intercept form submissions and apply the `was-validated` class. Without this, inline `.invalid-feedback` UI messages will not be triggered when validation fails.
**Action:** When using Bootstrap's `needs-validation` form class for inline error messages, always ensure the corresponding JavaScript is included on the page to intercept submissions and apply `was-validated`.
