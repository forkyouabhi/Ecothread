...
## 2024-10-24 - Bootstrap Form Validation Feedback
**Learning:** Bootstrap forms utilizing the `needs-validation` class require a custom client-side JavaScript snippet to intercept form submissions and apply the `was-validated` class; without this, inline `.invalid-feedback` UI messages will not be triggered.
**Action:** When using Bootstrap's `needs-validation` and `.invalid-feedback` elements, ensure the required client-side script is present to provide accessible, immediate error messaging instead of relying on default browser tooltips or silent failures.
