...
## 2024-05-20 - Inline Validation Needs Custom Interceptor
**Learning:** Bootstrap forms with the `needs-validation` class do not natively prevent submission or add `was-validated` automatically when forms are invalid.
**Action:** Always include a custom JavaScript interceptor snippet (like `validation.js`) to manually prevent submission and append the `was-validated` class to trigger `.invalid-feedback` UI.
