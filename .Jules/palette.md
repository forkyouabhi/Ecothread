...
## 2024-07-28 - Enhancing Forms with Required Indicators and Autofocus
**Learning:** Forms often lacked clear visual indication of required fields beyond browser defaults, causing potential confusion for users until submission. Additionally, users had to manually focus the first input field to start typing, adding a small amount of friction. Lack of autocomplete attributes on auth forms made password manager integration suboptimal.
**Action:** Always add visual indicators like an asterisk (e.g., `<span class="text-danger" aria-hidden="true" title="Required">*</span>`) for required fields. Use `autofocus` on the primary input of a form to streamline entry. Include appropriate `autocomplete` attributes (like `username`, `current-password`, `new-password`) to assist password managers.
