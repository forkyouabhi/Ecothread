...
## 2024-05-25 - Bootstrap Form Validation Requires Explicit JS
**Learning:** Forms utilizing Bootstrap's `needs-validation` class do not automatically trigger the `.invalid-feedback` UI messages upon submission. They require a specific client-side JavaScript snippet to intercept the submission, evaluate `checkValidity()`, and apply the `was-validated` class. Without this JS, the inline validation feedback remains invisible.
**Action:** When adding validation to Bootstrap forms, ensure the accompanying `was-validated` toggle JS script is included to activate the visual validation feedback.
